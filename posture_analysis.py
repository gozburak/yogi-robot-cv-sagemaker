...
class PostureAnalysis():
    ...
    def get_joints_params(allJoints):
        ...
        for allJoints_pp in allJoints:
            paramsJoints_pp = {}
            for joint_name in PostureAnalysis.keys_Joints:
    ...
    def build_body_from_joints(allJoints):
        ...
        for joint in allJoints:
            iter_body = {}
            for bodypart_name, joint_names in PostureAnalysis.BodyForm.items():
                body = PostureAnalysis.Bodypart(bodypart_name, joint[joint_names[0]], joint[joint_names[1]])
                body.get_metrics()
    ...
    def calculate_deviations(paramsBodyparts):
        ...
        for paramsBodyparts_pp in paramsBodyparts:
            ...
            for bodypart_name, data in paramsBodyparts_pp.items():
                diff = data['Angle'] - PostureAnalysis.ground_truth_angles[bodypart_name]['GT Angle']
    ...
    def create_json(pred_coords, confidence, bboxes, scores, client, iot_topic, session):
        ...
        allJoints = [{name: Joint(name, coord[0], coord[1], conf[0]) for name, coord, conf in
                      zip(PostureAnalysis.keys_Joints, coord_per_person, conf_per_person)} for coord_per_person, conf_per_person in
                     zip(pred_coords_clean, confidence_clean)]
        allBodyParts = PostureAnalysis.build_body_from_joints(allJoints)
        ...
        paramsJoints = PostureAnalysis.get_joints_params(allJoints)
        paramsBodyparts = PostureAnalysis.get_bodyparts_params(allBodyParts)

        # Time stamp is added to the output
        time_now = datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")

        # Calculating deviations
        deviations = PostureAnalysis.calculate_deviations(paramsBodyparts)
